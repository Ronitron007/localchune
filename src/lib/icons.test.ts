...
describe('the set', () => {
  it('exports thirteen glyphs and no more', () => {
    // Thirteen, not fifteen: `prev` is `next` mirrored with a CSS
    // transform and the status `x` is `close` reused. Two paths that must
    // stay visually consistent will eventually stop being consistent.
  ...
  it('names them exactly as the spec does', () => {
    expect(Object.keys(ICONS).sort()).toEqual([
      ...
      'next', 'pause', 'play', 'queue-add', 'search', 'upload',
    ])
  })
...
describe('paint discipline', () => {
  ...
  it.each(['queue-add', 'download', 'upload', 'search', 'close', 'crate', 'check'] as IconName[])(
