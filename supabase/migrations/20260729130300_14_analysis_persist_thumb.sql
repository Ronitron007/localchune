-- supabase/migrations/20260729130300_14_analysis_persist_thumb.sql
-- localchune — MIT licensed. See LICENSE.
-- NOTE: the distributed combination is AGPL-3.0 because the analysis
-- worker includes Essentia. LICENSE explains why.

-- analysis_persist() re-created verbatim from migration 09 with exactly one
-- change, applied in three places: it now stores `thumb_key` — the 64px
-- cover thumbnail the pool table renders (brutalist spec §4, worker task).
-- The column itself is added by migration 11 (pool_view), the artifact is
-- written by the analysis DO (workers/analysis/src/index.ts), and the
-- container emits it in worker/app/main.py. Without this, the DO uploads
-- derived/<file_id>/thumb.jpg and nothing ever records it, so has_thumb
-- stays false forever and every row renders the empty box.
--
-- create or replace preserves the function's ACL; the revoke/grant pair
-- from migration 09 is carried inside the extracted block below so this
-- file re-states its own security posture.
