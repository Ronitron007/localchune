...
export function maxBytesFor(c: Container): number {
  // Integer path deliberately. `* 1.1` is NOT exact in binary floating point:
  // it yields 63_360_001 rather than 63_360_000 for the 512 kbps containers,
  // which fails the boundary tests by one byte. `* 11 / 10` stays exact.
  const bytesNoHeadroom = ((MAX_DURATION_MS / 1000) * MAX_KBPS[c] * 1000) / 8
